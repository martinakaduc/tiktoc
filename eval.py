...
def evaluate(configs, now, test_set, lstm_inputs, tokenizer, device):
    ...
    if configs.save_model:
        ...
        if (
            ...
            model = AutoModelForCausalLM.from_pretrained(
                os.path.join(configs.model_save_dir, now, "model")
            ).to(device)
            linear = nn.Linear(configs.lstm_hid_dim, 768).to(device)

        linear.load_state_dict(
            torch.load(os.path.join(configs.model_save_dir, now, "linear"))
        )
        ...
        if configs.use_lstm:
            ...
            lstm.load_state_dict(
                torch.load(os.path.join(configs.model_save_dir, now, "lstm"))
            )
    ...
    for idx in tqdm(range(len(test_set)), desc="inference"):
        generated_code, ground_truth_code, prompt, student = generate_code(
            test_set, lstm_inputs, tokenizer, idx, model, lstm, linear, configs, device
        )
    ...
    ## compute codebleu
    codebleu_score, detailed_codebleu_score = compute_code_bleu(
        ground_truth_codes, generated_codes
    )
    ...
    if configs.save_model:
        with open(
            os.path.join(configs.model_save_dir, now, "eval_logs.pkl"), "wb"
        ) as f:
...
def generate_code(
    ...
    student, step, prompt, code = (
        test_set[idx]["SubjectID"],
        test_set[idx]["step"],
        test_set[idx]["next_prompt"],
        test_set[idx]["next_code"],
    )
    ks = get_knowledge_states_for_generator(
        lstm, lstm_inputs, [student], [step], configs, device, generation=True
    )

    # Get generator input
    inputs = tokenizer(
        build_prompt_with_special_tokens(prompt, tokenizer, configs),
        return_tensors="pt",
    )
    if (
        configs.okt_model != "codellama/CodeLlama-7b-Instruct-hf"
        ...
        inputs_embeds = model.base_model.model.model.embed_tokens(
...
def eval_student(configs, now, test_set, dataset, tokenizer, device):
    ...
    if configs.save_model:
        if (
            ...
            linear = nn.Sequential(
                nn.Linear(configs.lstm_hid_dim, 1600), nn.ReLU(), nn.Linear(1600, 4096)
            ).to(device)
        ...
        if configs.multitask:
            if configs.multitask_label != "granular":
                ...
                predictor.load_state_dict(
                    torch.load(os.path.join(configs.model_save_dir, now, "predictor"))
                )
            else:
                predictor = torch.load(
                    os.path.join(configs.model_save_dir, now, "predictor.pth")
                )
    ...
    collate_fn = CollateForOKTstudent(
        tokenizer=tokenizer,
        configs=configs,
        device=device,
        eval=True,
        question_test_dict=question_input_dict,
        question_no_map=question_no_map,
    )
    ...
    _, test_loader = make_dataloader(
        ...
        train=False,
        split_by_student=True,
        granular=granular,
        okt_model=True,
    )

    (
        generated_code_total,
        gt_code_total,
        prompt_total,
        pred_score_total,
        gt_score_total,
    ...
    for idx, batch in enumerate(tqdm(test_loader, desc="inference", leave=False)):
        if configs.multitask:
            ...
            generated_code_ls, gt_code_ls, prompt_ls, student_ls = (
                generate_code_student(
                    batch,
                    tokenizer,
                    model,
    ...
    generated_codes = [
        gen_code_i for code_ls in generated_code_total for gen_code_i in code_ls
    ]
    ...
    if configs.multitask:
        if configs.multitask_label != "granular":
            pred_scores = [
                pred_score_i
                for pred_subset in pred_score_total
                for pred_score_i in pred_subset
            ]
            gt_scores = [
                gt_score_i for gt_subset in gt_score_total for gt_score_i in gt_subset
            ]
            ...
            pred_res = sum(
                [pred == label for pred, label in zip(pred_labels, gt_scores)]
            )
...
def generate_code_student(
    batch, tokenizer, model, lstm, linear, configs, device, predict_linear=None
):
    (
        gen_code_ls,
        gt_code_ls,
    ...
    if (
        ...
        generator_input_wte = model.base_model.model.model.embed_tokens(
            padded_input_ids_ls
        ).to(device)
    ...
    padded_attention_mask_ls = padded_attention_mask_ls.to(
    ...
    flattened_prompt = [
        prompt_i for subprompt in padded_prompts for prompt_i in subprompt
    ]
    flattened_students = [
        student_i for substudent in padded_students for student_i in substudent
    ]
            ...
            if configs.multitask:
                ...
                if configs.multitask_inp != "concat":
                    if configs.multitask_label == "granular":
                        ...
                        predicted_label, predicted_score = predict_score_question_only(
                            input_wte_i,
                            attention_i,
                            model,
                            predict_linear,
                            granular=granular,
                            problem_seqs=prob_seq_sub,
                        )
                    else:
                        predicted_score = predict_score_question_only(
                            input_wte_i,
                            attention_i,
                            model,
                            predict_linear,
                    ...
                    predicted_score = predict_score_concat(
                        pred_input_subset[i], predict_linear
                    )
...
def predict_score_question_only(
    generator_input_wte,
    attention_mask,
    model,
    predict_linear,
    ...
    with torch.no_grad():
        output = model(
            inputs_embeds=generator_input_wte,
            attention_mask=attention_mask,
...
def batch_generate(
    test_set, lstm_inputs, tokenizer, model, lstm, linear, configs, device
):
    prompts = [test_set[i]["next_prompt"] for i in range(len(test_set))]
    ground_truch_codes = [
        test_set[i]["next_code"].strip() for i in range(len(test_set))
    ]
    ...
    ks_batch = [
        get_knowledge_states_for_generator(
            lstm,
            lstm_inputs,
            [test_set[i]["SubjectID"]],
            [test_set[i]["step"]],
            configs,
            device,
            generation=True,
...
def batch_inference(configs, now, tokenizer, test_set, lstm_inputs, device):
    ...
    for idx in tqdm(range(0, len(test_set), 20), desc="batch inference"):
        ...
        generated_code_i, ground_truth_code_i, prompts_i, students_i = batch_generate(
            test_set_i, lstm_inputs, tokenizer, model, lstm, linear, configs, device
        )
...
def eval_granular(configs, test_loader, device, now, loss_function):
    if configs.save_model:
        ...
        if configs.use_transition_model:
            ...
            transition_model.load_state_dict(
                torch.load(os.path.join(configs.model_save_dir, now, "transition"))
            )
        else:
            transition_model = None
        granular_model = torch.load(
            os.path.join(configs.model_save_dir, now, "granular_model.pth")
        )
    ...
    for idx, batch in enumerate(tqdm(test_loader, desc="inferece", leave=False)):
        test_log, pred_ls = predict_granular_step(
            idx,
            batch,
            lstm,
            granular_model,
        ...
        if configs.split_by == "submission":
            if configs.save_model:
                ...
                if (
                    ...
                    model = okt_model_init(
                        configs, device, now, False, load_in_8bit=True
                    )
            collate_fn = CollateForOKT(
                tokenizer=tokenizer, configs=configs, device=device
            )
            train_set, valid_set, test_set, dataset, students = read_data(
                configs, tokenizer, model, device
            )
            ...
            train_set, valid_set, test_set, dataset, students = read_granular_data(
                configs
            )
