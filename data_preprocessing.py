    ...
    for q_i in questions:
        ...
        if (
            ...
            token_embedding = model.transformer.wte(
                prompt_embedding["input_ids"].to(device)
            )
        else:
...
def construct_dataset(configs, tokenizer, model, device):
    ...
    dataset["input"] = dataset[["prompt-embedding", "embedding"]].apply(
...
def handle_score_mismatch(configs):
    ...
    for i in range(7):
        ...
        for student, info in dataset.items():
            ...
            for code, bi_score, match, prompt, score in zip(
                codes, bi_scores, match_ls, prompts, scores
            ):
                if not match:
                    cnt += 1
    ...
    final_dict = {
        "ground_truth_codes": gt_code_ls,
        "prompts": prompt_ls,
        "students": student_ls,
        "scores": score_ls,
        "binary_correctness": bi_score_ls,
    }
...
def preprocess_all_submission(configs, orig_df):
    ...
    combined_df = combined_df.drop_duplicates(
        subset=["SubjectID", "Code", "timestep"], keep="first"
    )
...
def match_timestep(configs, tokenizer, model, device):
    ...
    combined_df = pd.merge(
        dataset,
        filtered,
        on=["SubjectID", "AssignmentID", "ProblemID", "CodeStateID"],
        how="left",
    )
