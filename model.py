...
def create_lstm_model(configs, device):
    ...
    lstm = nn.LSTM(
        configs.lstm_inp_dim, configs.lstm_hid_dim, num_layers=configs.num_layers
    )
    lstm.to(device)
...
def create_okt_model(configs, device):
    ...
    if configs.okt_model == "student":
        ...
        model = AutoModelForCausalLM.from_pretrained(
            configs.okt_model, quantization_config=bnb_config
        )
...
def okt_model_init(configs, device, now, continue_train, load_in_8bit=True):
    ...
    model = PeftModel.from_pretrained(
        _hf_model, model_dir, is_trainable=continue_train
    ).to(device)
...
def create_granular_model(configs, device):
    ...
    if configs.use_transition_model:
        model = nn.Parameter(
            torch.empty(
                (configs.valid_question_no, configs.trans_hid_dim, configs.T_max)
            ).to(device),
            requires_grad=True,
        )
    else:
        if configs.multitask_label:
            model = nn.Parameter(
                torch.empty((configs.valid_question_no, prompt_dim, configs.T_max)).to(
                    device
                ),
                requires_grad=True,
            )
            ...
            if configs.use_lstm:
                model = nn.Parameter(
                    torch.empty(
                        (
                            configs.valid_question_no,
                            configs.lstm_hid_dim + prompt_dim,
                            configs.T_max,
                        )
                    ).to(device),
                    requires_grad=True,
                ...
                model = nn.Parameter(
                    torch.empty(
                        (configs.valid_question_no, prompt_dim, configs.T_max)
                    ).to(device),
                    requires_grad=True,
                )
...
def create_transition_layer(configs, device):
    ...
    transition_layer = nn.Sequential(
        nn.Linear(configs.lstm_hid_dim + prompt_dim, configs.trans_hid_dim), nn.ReLU()
    )
...
def create_multi_linear_with_emd(
    device,
    tokenizer,
    model,
    question_dict,
    ...
    for key in question_dict.keys():
        ...
        token_embedding = model.base_model.model.model.embed_tokens(
            test_embedding["input_ids"].to(device)
        )
