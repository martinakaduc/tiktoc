...
def split_large_dataset(configs, baseline):
    ...
    with open(path, "rb") as f:
        ...
        for i in range(0, length, 2000):
            code_pair_sub = {}
            code_pair_sub["ground_truth_codes"] = data["ground_truth_codes"][
                i : i + 2000
            ]
        ...
        for i in range(len(students)):
            ...
            if prompt_i in all_good_question:
                ...
                if not baseline:
                    ...
                    student_code_pair[student_i]["generated_codes"].append(
                        generated_code_i
                    )
                else:
                ...
                student_code_pair[student_i]["ground_truth_codes"].append(
                    ground_truth_code_i
                )
                # cnt += 1
...
def create_unit_test_case_function(function_name, test_cases_list):
    ...
    """
    unit_test_case_function_code = "public void {:s}_test(String []s)".format(
        function_name
    )
...
def get_complete_java_code(
    class_name, code, unit_test_case_function_code, main_function_code
):
    """
...
def construct_java_code(
    test_cases, function_name, class_name, student_code, output_type
):
    """
    Constructs the full Java code
    """
    # create a unit test case function
    unit_test_case_function_code = create_unit_test_case_function(
    ...
    complete_java_code = get_complete_java_code(
        class_name,
        exception_handeled_code,
        unit_test_case_function_code,
        main_function_code,
    )
    return complete_java_code
...
def save_code(group, code, p):
    ...
    if not os.path.exists(
        "compiler_code/{:d}_{:d}".format(group_tuple[0], group_tuple[1])
    ):
        os.mkdir("compiler_code/{:d}_{:d}".format(group_tuple[0], group_tuple[1]))
    with open(
        "compiler_code/{:d}_{:d}/{:s}.java".format(group_tuple[0], group_tuple[1], p),
        "w",
    ) as f:
        f.write(code)
...
def run_command(java_code_group_path, code_name, stop_event):
    ...
    while not stop_event.is_set():
        try:
            os.system(
                "java {:s}/{:s}.java output > {:s}/output/output_{:s}.txt".format(
                    java_code_group_path, code_name, java_code_group_path, code_name
                )
            )
...
def get_score(solution, student_answer):
    ...
    acc_list = [
        1 if solution[i] == student_answer[i] else 0 for i in range(len(solution))
    ]
...
def get_granular_correctness(solution, student_answer, generated_answer):
    ...
    acc_gen = [
        1 if solution[i] == generated_answer[i] else 0 for i in range(len(solution))
    ]
    granular_score = sum(1 for x, y in zip(acc_student, acc_gen) if x == y) / len(
        solution
    )
...
def is_valid_code(code):
    ...
    if (
        code_parts.count("int") > 200
        or code_parts.count("String") > 200
        or code_parts.count("boolean") > 200
        or len(code_parts) < 4
    ):
        is_valid = False
    return is_valid
...
def get_test_result_for_generated_code(
    good_test_case, student_code_pair, desc_id_pair, metric="MSE", eval=True
):
    ...
    gt_function_name_dict = {
        1: ("sortaSum", "int"),
        3: ("in1To10", "boolean"),
        5: ("answerCell", "boolean"),
        12: ("squirrelPlay", "boolean"),
        13: ("caughtSpeeding", "int"),
        17: ("redTicket", "int"),
        20: ("loneSum", "int"),
        21: ("luckySum", "int"),
        22: ("noTeenSum", "int"),
        24: ("blackjack", "int"),
        25: ("evenlySpaced", "boolean"),
        34: ("zipZap", "String"),
        37: ("endOther", "boolean"),
        39: ("xyBalance", "boolean"),
        40: ("getSandwich", "String"),
        46: ("isEverywhere", "boolean"),
        71: ("canBalance", "boolean"),
    }
    ...
    if eval:
        ...
        group_cases_dict = {
            0: {
                0: {"MSE": 0.0, "cnt": 0, "granular": 0.0, "stats_ls": []},
                1: {"MSE": 0.0, "cnt": 0, "granular": 0.0, "stats_ls": []},
                2: {"MSE": 0.0, "cnt": 0, "granular": 0.0, "stats_ls": []},
            },
            1: {
                0: {"MSE": 0.0, "cnt": 0, "granular": 0.0, "stats_ls": []},
                1: {"MSE": 0.0, "cnt": 0, "granular": 0.0, "stats_ls": []},
    ...
    for student in student_code_pair.keys():
        ...
        for i in range(len(question_code_pair["prompts"])):
            ...
            if eval:
                ...
                method_name, output_type = extract_function_feature_complete(
                    generated_code_i, method_name_gt
                )
                ...
                if valid_gen:
                    valid_cnt += 1
                    full_part = construct_java_code(
                        spec_test_case,
                        method_name,
                        method_name.upper() + student,
                        generated_code_i,
                        output_type,
                    )

                    save_code(group_i, full_part, method_name.upper() + student)
                    output_i = execute_and_store_update(
                        group_i, method_name.upper() + student
                    )
            ...
            full_part_gt = construct_java_code(
                spec_test_case,
                method_name_gt,
                method_name_gt.upper() + student + "_gt",
                gt_code_i,
                output_type_gt,
            )
            ...
            output_i_gt = execute_and_store_update(
                group_i, method_name_gt.upper() + student + "_gt"
            )
            ...
            accuracy_gt, bi_acc_ls_gt = get_score(
                solution_i, student_ground_truth_i[:-1]
            )
            ...
            if eval:
                ...
                if (
                    len(student_ground_truth_i[:-1]) > 0
                    and len(student_answer_i[:-1]) > 0
                ):
                    mse_all_compile += (accuracy_gt - accuracy) ** 2
                    granular_error_compile += get_granular_correctness(
                        solution_i, student_ground_truth_i[:-1], student_answer_i[:-1]
                    )
                    all_compile_cnt += 1
                    ...
                    granular_error_gen_compile += get_granular_correctness(
                        solution_i, student_ground_truth_i[:-1], student_answer_i[:-1]
                    )
                    gen_compile_cnt += 1
                    ...
                    granular_error_gt_compile += get_granular_correctness(
                        solution_i, student_ground_truth_i[:-1], student_answer_i[:-1]
                    )
                    gt_compile_cnt += 1
                    ...
                    granular_error_no_compile += get_granular_correctness(
                        solution_i, student_ground_truth_i[:-1], student_answer_i[:-1]
                    )
                    no_compile_cnt += 1
                ...
                if gt_status == 2 and gen_status == 0:
                    both_pass_debug_ls.append(
                        (group_i, method_name_gt.upper() + student)
                    )
    ...
    if eval:
        ...
        acc = accuracy = sum(
            [1 for true, pred in zip(final_gt, final_compiled) if true == pred]
        ) / len(final_gt)
        print("Accuracy:", acc)
    ...
    for student in student_code_pair.keys():
        ...
        for i in range(len(question_code_pair["prompts"])):
            ...
            if cnt > target_cnt - 3 and cnt < target_cnt + 3:
                student_code_temp[student]["prompts"].append(
                    question_code_pair["prompts"][i]
                )
                student_code_temp[student]["ground_truth_codes"].append(
                    question_code_pair["ground_truth_codes"][i]
                )
...
def generated_code_error_analyze(gen_error_ls):
    ...
    print(
        "Average errors on generated error code without mismatch:",
        (error_cnt - cannot_apply_cnt) / len(gen_error_ls),
    )
...
def main():
    ...
    student_code_pair, desc_id_pair = validate_prompt_test_set(
        configs, all_good_test, now=now
    )

    student_code_pair = get_test_result_for_generated_code(
        good_test_case, student_code_pair, desc_id_pair, metric="granular", eval=True
    )
