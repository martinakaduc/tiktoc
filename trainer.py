...
def generator_step(
    ...
    generator_inputs_wte, ks = assemble_generator_input(
        model,
        lstm,
        linear,
        configs,
    ...
    if train:
        outputs = model(
            inputs_embeds=generator_inputs_wte,
...
def generator_student_step(
    ...
    if (
        ...
        generator_input_wte = model.transformer.wte(
            padded_input_ids_ls
        )  # Shape = [B, T, generator_inputs_ids.shape(1), 768]
    else:
    ...
    range_tensor = (
        torch.arange(padded_input_ids_ls.size(2), device=device)
        .unsqueeze(0)
        .unsqueeze(0)
    )  # Shape = [1, 1, max_length]
    range_tensor = range_tensor.repeat(
        padded_input_ids_ls.size(0), padded_input_ids_ls.size(1), 1
    )  # Shape = [T, B, max_length]
    mask_tensor = range_tensor >= padded_prompt_id_lens_ls.unsqueeze(
        -1
    )  # Shape = [T, B, max_length]
    ...
    if configs.use_lstm:
        ...
        ks = ks.unsqueeze(2).repeat(
            1, 1, padded_input_ids_ls.size(2), 1
        )  # (T, B, max_length, hidden_dim)
        ks[mask_tensor] = torch.zeros(
            ks.size(-1), device=device
        )  # Shape = [T, B, max_length, 4096]

        generator_input_wte = torch.add(
            generator_input_wte, ks
        )  # Shape = [T, B, max_length, 4096]
    ...
    for i in range(len(input_wte_groups)):
        ...
        if train:
            ...
            ) and configs.first_ast_convertible:
                outputs = model(
                    inputs_embeds=input_wte_sub,
        ...
        if multitask:
            ...
            if configs.multitask_label == "granular":
                ...
                pred_loss_sub = pred_loss_fn(
                    logits[non_padding_mask], score_sub[non_padding_mask]
                ).sum()
                ...
                pred_loss_sub = pred_loss_fn(
                    logits[score_sub != -100], score_sub[score_sub != -100]
                ).sum()
                pred_cum_loss += pred_loss_sub
    ...
    if configs.multitask:
        ...
        if configs.multitask_label == "granular":
            pred_res = (
                pred_total[gt_total != -100].detach().cpu()
                == gt_total[gt_total != -100].detach().cpu()
            )
            log["acc"] = pred_res
            log["auc"] = {
                "logits": logits_total[gt_total != -100].detach().cpu(),
...
def get_knowledge_states_for_generator(
    lstm, lstm_inputs, students, timesteps, configs, device, generation=False
):
    """
    ...
    if configs.use_lstm:
        ...
        padded_lstm_ins = [
            i + [torch.zeros(i[0].shape[0])] * (max_len - len(i)) for i in lstm_ins
        ]
        padded_lstm_ins = torch.stack(
            [torch.stack(x, dim=0) for x in padded_lstm_ins], dim=1
        ).float()  # Shape = [T, B, D_bar]
        # Get student knowledge states
        if configs.train_lstm and not generation:
            out, hidden = lstm(
                padded_lstm_ins.to(device)
            )  # Shape = [T, B, D_bar], D_bar = lstm_hid_dim
        else:
            with torch.no_grad():
                out, hidden = lstm(
                    padded_lstm_ins.to(device)
        ...
        ks = out[
            timesteps, list(range(out.shape[1])), :
        ]  # Extract the hidden states -> shape = [B, D_bar]

    return ks


def assemble_generator_input(
    model,
    lstm,
    linear,
    configs,
    ...
    if (
        ...
        generator_input_wte = model.transformer.wte(
            generator_inputs_ids
        )  # Shape = [B, generator_inputs_ids.shape(1), 768]
    else:
        generator_input_wte = model.base_model.model.model.embed_tokens(
            generator_inputs_ids
        )  # Shape = [B, generator_inputs_ids.shape(1), 4096]

    # get knowledge states
    ks = get_knowledge_states_for_generator(
        lstm, lstm_inputs, students, timesteps, configs, device, generation
    )
    ...
    ks = ks.unsqueeze(1).repeat(
        1, generator_input_wte.size(1), 1
    )  # Shape = [B, T, 768], T refers to max_input_length
    range_tensor = torch.arange(generator_inputs_ids.size(1), device=device).unsqueeze(
        0
    )  # Shape = [1, T]
...
def predict_granular_step(
    idx,
    batch,
    lstm,
    granular_model,
    transition_model=None,
    optimizers_granular=None,
    optimizers_lstm=None,
    optimizers_trans=None,
    configs=None,
    loss_fn=None,
    train_dl_len=None,
    train=True,
    ...
    padded_granular_cor, padded_inputs, padded_embeddings, padded_question_seqs = (
        batch[0][1:].to(device),
        batch[1][:-1].to(device),
        batch[2][1:].to(device),
        batch[3][1:].to(device),
    ...
    log = {
        "loss": loss.detach().cpu().true_divide(len(label[label != -100])),
        "acc": pred_res,
        "auc": {
            "logits": logits[label != -100].detach().cpu(),
            "scores": label[label != -100].detach().cpu(),
        },
    }
    ...
    if eval:
        ...
        mse = (
            torch.square(torch.subtract(label_scores, pred_scores))
            .mean()
            .detach()
            .cpu()
        )
