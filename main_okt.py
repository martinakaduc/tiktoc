...
from eval import *
from gen_code_compiler import (
    get_test_case_solution,
    handle_uniq_test_exception,
    test_case_check,
    uniq_test_construct,
)
...
def sanitize_configs(configs):
    assert (configs.use_lstm == False and configs.lstm_hid_dim == 0) or (
        configs.use_lstm == True and configs.lstm_hid_dim > 0
    ), "Invalid combination of configs use_lstm and lstm_hid_dim"
...
def main(configs):
    ...
    if configs.split_by == "submission":
        ...
        train_set, valid_set, test_set, dataset = construct_okt_dataset_from_granular(
            configs
        )
        ...
        _, train_loader, lstm_inputs = make_dataloader(
            train_set,
            dataset,
            collate_fn=collate_fn,
            configs=configs,
        ...
        _, valid_loader = make_dataloader(
            valid_set,
            None,
            collate_fn=collate_fn,
            configs=configs,
            do_lstm_dataset=False,
            train=False,
        )
        ...
        if configs.multitask_label == "granular":
            ...
            if configs.multitask_init_combine:
                question_prompt = (
                    dataset.sort_values(by="ProblemID").prompt.unique().tolist()
                )
                quest_prompt_dict = {
                    question_ids[i]: question_prompt[i]
            ...
            if configs.multitask_init != "emb":
                ...
                predictor = create_multi_linear_with_emd(
                    device,
                    tokenizer,
                    model,
                    question_input_dict,
                    solution,
                    question_in=configs.multitask_init_combine,
                    question_prompt_dict=quest_prompt_dict,
                )
    ...
    if configs.continue_training:
        ...
        optimizer_lm.load_state_dict(
            torch.load(os.path.join(configs.model_save_dir, now, "optimizer_lm.pth"))
        )
        optimizers_generator.append(optimizer_lm)

        optimizer_linear = optim.AdamW(linear.parameters(), lr=configs.lr_linear)
        optimizer_linear.load_state_dict(
            torch.load(
                os.path.join(configs.model_save_dir, now, "optimizer_linear.pth")
            )
        )
        optimizers_generator.append(optimizer_linear)

        optimizer_lstm = optim.RMSprop(
            lstm.parameters(), lr=configs.lstm_lr, momentum=0.9
        )
        optimizer_lstm.load_state_dict(
            torch.load(os.path.join(configs.model_save_dir, now, "optimizer_lstm.pth"))
        )
        optimizers_lstm = []
        ...
        if configs.multitask:
            ...
            if configs.multitask_label == "raw":
                optimizer_predictor = optim.AdamW(
                    predictor.parameters(), lr=configs.multitask_pred_linear
                )
            else:
                optimizer_predictor = optim.AdamW(
                    [predictor], lr=configs.multitask_pred_linear
                )
    ...
    scheduler = transformers.get_linear_schedule_with_warmup(
        optimizer_lm, num_warmup_steps, num_training_steps
    )
    if configs.continue_training:
    ...
    for ep in tqdm(range(configs.start_epoch, configs.epochs), desc="epochs"):
        ...
        for idx, batch in enumerate(tqdm(train_loader, desc="training", leave=False)):
            ...
            # New generator_step, split by student
                ...
                train_log = generator_student_step(
                    idx,
                    batch,
                    model,
                    lstm,
                    ...
                    scheduler=scheduler,
                    device=device,
                    multitask=configs.multitask,
                    predictor=predictor,
                    pred_loss_fn=multitask_loss_fn,
                    optimizers_multitask=optimizers_predictor,
                )
            ...
            if configs.log_train_every_itr and configs.log_wandb:
                if (idx + 1) % configs.log_train_every_itr == 0:
                    ...
                    for key in itr_train_logs:
                        wandb.log(
                            {
                                "metrics/train_every_{}_itr/{}".format(
                                    configs.log_train_every_itr, key
                                ): itr_train_logs[key]
                            }
                        )
        ...
        for idx, batch in enumerate(tqdm(valid_loader, desc="validation", leave=False)):
            if configs.split_by == "submission":
                ...
                valid_log = generator_student_step(
                    idx,
                    batch,
                    model,
                    lstm,
        ...
        for idx, batch in enumerate(tqdm(test_loader, desc="testing", leave=False)):
            if configs.split_by == "submission":
                ...
                test_log = generator_student_step(
                    idx,
                    batch,
                    model,
                    lstm,
            ...
            if key == "loss":
                if float(valid_logs[key]) < best_valid_metrics[key]:
                    ...
                    for key_best_metric in best_metrics_with_valid:
                        best_metrics_with_valid[key_best_metric] = float(
                            test_logs[key_best_metric]
                        )
                    ...
                    if configs.save_model:
                        if configs.log_wandb:
                            wandb.log(
                                {
                                    "best_model_at_epoch": ep,
                                    "best_valid_loss": best_valid_metrics[key],
                                }
                            )
                        ...
                        torch.save(
                            linear.state_dict(),
                            os.path.join(configs.model_save_dir, now, "linear"),
                        )
                        ...
                        if configs.use_lstm:
                            ...
                            torch.save(
                                lstm.state_dict(),
                                os.path.join(configs.model_save_dir, now, "lstm"),
                            )
                            optimizer_lstm_dir = os.path.join(
                                configs.model_save_dir, now, "optimizer_lstm.pth"
                            )
                        ...
                        if configs.multitask:
                            if configs.multitask_label == "granular":
                                torch.save(
                                    predictor,
                                    os.path.join(
                                        configs.model_save_dir, now, "predictor.pth"
                                    ),
                                )
                            else:
                                torch.save(
                                    ...
                                    os.path.join(
                                        configs.model_save_dir, now, "predictor"
                                    ),
                                )
                                optimizer_predictor_dir = os.path.join(
                                    configs.model_save_dir,
                                    now,
                                    "optimizer_predictor.pth",
                                )
                                torch.save(
                                    optimizer_predictor.state_dict(),
                                    optimizer_predictor_dir,
                                )

                        scheduler_dir = os.path.join(
                            configs.model_save_dir, now, "scheduler.pth"
                        )
                        torch.save(scheduler.state_dict(), scheduler_dir)

                        optimizer_lm_dir = os.path.join(
                            configs.model_save_dir, now, "optimizer_lm.pth"
                        )
                        torch.save(optimizer_lm.state_dict(), optimizer_lm_dir)

                        optimizer_linear_dir = os.path.join(
                            configs.model_save_dir, now, "optimizer_linear.pth"
                        )
        ...
        if configs.log_wandb:
            ...
            for key in best_metrics_with_valid:
                saved_stats["metrics/test/best_" + key + "_with_valid"] = (
                    best_metrics_with_valid[key]
                )
