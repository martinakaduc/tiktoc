    ...
    with open(os.path.join(configs.data_path, file_path), "rb") as f:
        ...
        for student, info in data.items():
            ...
            for code, bi_score in zip(codes, bi_scores):
                final_ls.append(
                    {"SubjectID": student, "Code": code, "binary_score": bi_score}
                )
...
def read_data(configs, tokenizer, model, device):
    ...
    dataset["SubjectID"] = [
        dataset.iloc[i].SubjectID + "_{}".format(dataset.iloc[i].SubjectID_appendix)
    ...
    if configs.split_by == "student":
        train_dkt, test_dkt = train_test_split(
            students, test_size=configs.test_size, random_state=configs.seed
        )
        valid_dkt, test_dkt = train_test_split(
            test_dkt, test_size=0.5, random_state=configs.seed
        )
    ...
    dropped_dataset = dropped_dataset.drop(
        dropped_dataset.index[dropped_dataset["timestep"] == 0]
    ).reset_index(drop=True)

    # Split on entries instead of on students
    trainset, testset = train_test_split(
        dropped_dataset, test_size=configs.test_size, random_state=configs.seed
    )
    validset, testset = train_test_split(
        testset, test_size=0.5, random_state=configs.seed
    )
    ...
    if do_lstm_dataset:
        if split_by_student:
            ...
            for student in students:
                ...
                subset.loc[:, "prompt-embedding"] = subset["prompt-embedding"].apply(
            ...
            for student in students:
                lstm_dataset[student] = dataset_full[
                    dataset_full.SubjectID == student
                ].input.tolist()
            del dataset_full
...
def make_dataloader(
    ...
    if do_lstm_dataset:
        if split_by_student:
            lstm_student = make_pytorch_dataset(
                dataset_split,
                dataset_full,
                do_lstm_dataset,
                split_by_student,
            ...
            okt_dataset, lstm_dataset = make_pytorch_dataset(
                dataset_split, dataset_full, do_lstm_dataset
            )
            data_loader = torch.utils.data.DataLoader(
                okt_dataset,
                collate_fn=collate_fn,
...
def get_lstm_inputs(configs, train_set, dataset, collate_fn):
    ...
    _, _, lstm_inputs = make_dataloader(
        train_set, dataset, collate_fn=collate_fn, configs=configs, do_lstm_dataset=True
    )
...
def construct_okt_dataset_from_granular(configs):
    ...
    valid_student_set, test_student_set = train_test_split(
        test_student_set, test_size=0.5, random_state=configs.seed
    )

    train_dataset = dataset[dataset["SubjectID"].isin(train_student_set)]
    dropped_train_set = train_dataset.drop(
        train_dataset.index[train_dataset["timestep"] == 0]
    ).reset_index(drop=True)

    valid_dataset = dataset[dataset["SubjectID"].isin(valid_student_set)]
    dropped_valid_set = valid_dataset.drop(
        valid_dataset.index[valid_dataset["timestep"] == 0]
    ).reset_index(drop=True)

    test_dataset = dataset[dataset["SubjectID"].isin(test_student_set)]
    dropped_test_set = test_dataset.drop(
        test_dataset.index[test_dataset["timestep"] == 0]
    ).reset_index(drop=True)
...
def get_test_case_solution():
    prompt_df = pd.read_csv(
        os.path.join("test-case-query-results/prompt_concept_summary.csv"),
        on_bad_lines="warn",
    )
    all_good = prompt_df.loc[prompt_df["Test Case Status"] == "All Good"]

    test_info_df = pd.read_csv(
        os.path.join("test-case-query-results/test_cases-1-26-24.csv"),
        on_bad_lines="skip",
    )
    ...
    df_q37 = pd.read_csv(
        os.path.join("test-case-query-results/test_case_37.csv"), on_bad_lines="warn"
    )
...
def handle_uniq_test_exception(question_input_dict):
    ...
    df_q37["total_input"] = df_q37[["input_1", "input_2"]].apply(
        lambda x: ", ".join(x[x.notnull()]).rstrip('"').replace("\\", ""), axis=1
    )
...
def map_test_case_to_dataset(solution_dict, question_input_dict, configs):
    ...
    dataset_testcase = dataset.explode(
        ["binary_score", "test_inputs", "test_solutions"]
    )
    dataset_testcase = dataset_testcase.drop(
        dataset_testcase.index[dataset_testcase["timestep"] == 0]
    ).reset_index(drop=True)

    trainstudent, teststudent = train_test_split(
        students, test_size=configs.test_size, random_state=configs.seed
    )
    validstudent, teststudent = train_test_split(
        teststudent, test_size=0.5, random_state=configs.seed
    )
...
def make_pytorch_testcase_dataset(dataset):
    ...
    subset.rename(
        columns={
            ...
            "prompt": "next_prompt",
            "Code": "next_code",
            "test_inputs": "next_test_input",
            "test_solutions": "next_test_solution",
            "binary_score": "next_binary_score",
        },
        inplace=True,
    )
...
def build_input_with_special_tokens(prompt, code, tokenizer, configs, ins_ft=None):
    ...
    input = (
        build_prompt_with_special_tokens(prompt, tokenizer, configs, ins_ft=ins_ft)
        + " "
        + code.strip()
        + tokenizer.eos_token
    )
...
def build_prompt_with_special_tokens(prompt, tokenizer, configs, ins_ft=None):
    ...
    if (
        configs.okt_model != "codellama/CodeLlama-7b-Instruct-hf"
        and configs.okt_model != "meta-llama/Meta-Llama-3-8B-Instruct"
        and configs.okt_model != "Qwen/Qwen1.5-7B"
    ):
...
def build_input_for_binary_res(
    prompt, test_input, test_output, res, tokenizer, ins_ft=None
):
    result = "pass" if res else "fail"
    input = (
        build_prompt_for_bianry_res(prompt, test_input, test_output, ins_ft=ins_ft)
        + " "
        + result
        + tokenizer.eos_token
    )
...
def build_instruction_prefix(task):
    instruction = (
        "Test case result prediction task: "
        if task == "tc"
        else "Code generation task: "
    )
    common = "You are simulating a student learning to program in Java. "
    task = (
        "Given an input problem and associated test case, predict whether the code written by the student will pass or fail the test case. "
        if task == "tc"
        else "Given an input problem, predict the code written by the student. "
    )
...
def find_max_token_length(tokenizer, inputs_ids, attention_mask, prompt_id_lens):
    padding_ids = torch.where(
        inputs_ids == tokenizer.convert_tokens_to_ids(tokenizer.eos_token), 1, 0
    )
    ...
    def __call__(self, batch):
        inputs_text = [
            build_input_with_special_tokens(
                b["next_prompt"], b["next_code"], self.tokenizer, self.configs
            )
            for b in batch
        ]
        ...
        inputs_ids, attention_mask = inputs["input_ids"].to(self.device), inputs[
            "attention_mask"
        ].to(self.device)
        ...
        self.max_length_label = find_max_token_length(
            self.tokenizer, inputs_ids, attention_mask, prompt_id_lens
        )
...
class CollateForOKTstudent(object):
    ...
    def __call__(self, batch):
        if self.configs.multitask_label != "granular":
            ...
            padded_scores = (
                torch.Tensor(padded_scores).t().to(self.device)
            )  # shape: (T, B)
            ...
            padded_scores = [
                [
                    test_case + [-100] * (self.T_max - len(test_case))
                    for test_case in ite
                ]
                for ite in scores
            ]
            padded_scores = [
                i + [[-100] * self.T_max for _ in range(max_len - len(i))]
                for i in padded_scores
            ]
            padded_scores = torch.Tensor(
                padded_scores
            ).float()  # padded_granular_scores shape: (B, T, T_max)
            padded_scores = torch.transpose(
                padded_scores, 0, 1
        ...
        padded_inputs = [
            i + [torch.zeros(i[0].shape[0])] * (max_len - len(i)) for i in inputs
        ]
        padded_inputs = (
            torch.stack([torch.stack(x, dim=0) for x in padded_inputs], dim=1)
            .float()
            .to(self.device)
        )  # shape: (T, B, D)
                    ...
                    [
                        self.tokenizer.apply_chat_template(
                            [
                                {
                                    "role": "system",
                                    "content": "You are simulating a student learning to program in Java. Given an input problem, predict the code written by the student.",
                                },
                                {
                                    "role": "user",
                                    "content": build_prompt_with_special_tokens(
                                        prompt_i, self.tokenizer, self.configs
                                    ),
                                },
                        ...
                        for prompt_i in entry["next_prompt"]
                    ]
                    for entry in batch
                ]
            else:
            ...
            if self.configs.use_template:
                input_texts = [
                    [
                        ...
                        for prompt_i, code_i in zip(
                            entry["next_prompt"], entry["next_code"]
                        )
                    ]
                    for entry in batch
                ]
                ...
                input_texts = [
                    [
                        build_input_with_special_tokens(
                            prompt_i, code_i, self.tokenizer, self.configs
                        )
                        for prompt_i, code_i in zip(
                            entry["next_prompt"], entry["next_code"]
                        )
        ...
        for input_sub in input_texts:
            ...
            if not self.configs.use_template:
                inputs = self.tokenizer(
                    input_sub, return_tensors="pt", padding=True, truncation=True
                )
                inputs_ids, attention_mask = inputs["input_ids"].to(
                    self.device
                ...
                delimiter_indices = torch.where(
                    inputs_ids == self.delimiter_token_id, 1, 0
                )
                if self.configs.okt_model == "Qwen/Qwen1.5-7B":
                    delimiter_indices[:, 1] = 0
                else:
                ...
                delimiter_indices = torch.where(
                    inputs_ids == self.tokenizer.convert_tokens_to_ids("assistant"),
                    1,
                    0,
                )
                prompt_id_lens = torch.argmax(delimiter_indices, dim=-1)
            ...
            range_tensor = torch.arange(
                inputs_ids.size(1), device=self.device
            ).unsqueeze(0)
            range_tensor = range_tensor.repeat(prompt_id_lens.size(0), 1)
            mask_tensor = range_tensor < prompt_id_lens.unsqueeze(-1)
            labels[mask_tensor] = -100
        ...
        padded_input_ids_ls = [
            ...
            for input_ids in inputs_ids_ls
        ]
        padded_input_ids_ls = pad_sequence(
            padded_input_ids_ls,
            batch_first=True,
            padding_value=self.tokenizer.eos_token_id,
        )
        padded_input_ids_ls = torch.transpose(
            padded_input_ids_ls, 0, 1
        )  # shape: (T, B, max_length)

        padded_attention_mask_ls = [
            torch.nn.functional.pad(
                attention_mask, (0, max_length - attention_mask.shape[1]), value=0
            )
            for attention_mask in attention_mask_ls
        ]
        padded_attention_mask_ls = pad_sequence(
            padded_attention_mask_ls, batch_first=True, padding_value=0
        )
        padded_attention_mask_ls = torch.transpose(
            padded_attention_mask_ls, 0, 1
        )  # shape: (T, B, max_length)

        padded_labels_ls = [
            torch.nn.functional.pad(
                labels, (0, max_length - labels.shape[1]), value=-100
            )
            for labels in labels_ls
        ]
        padded_labels_ls = pad_sequence(
            padded_labels_ls, batch_first=True, padding_value=-100
        )
        padded_labels_ls = torch.transpose(
            padded_labels_ls, 0, 1
        )  # shape: (T, B, max_length)

        padded_prompt_id_lens_ls = [
            torch.cat((i, torch.zeros(max_len - i.size(0)).to(self.device)), 0)
            for i in prompt_id_lens_ls
        ]
        padded_prompt_id_lens_ls = torch.stack(
            padded_prompt_id_lens_ls
        ).t()  # shape: (T, B)
...
class CollateForGranularDKT(object):
    ...
    def __call__(self, batch):
        ...
        padded_inputs = torch.stack(
            [torch.stack(x, dim=0) for x in padded_inputs], dim=1
        ).float()

        test_case_cors = [b["granular_correctness"] for b in batch]

        padded_granular_cor = [
            [test_case + [-100] * (self.T_max - len(test_case)) for test_case in ite]
            for ite in test_case_cors
        ]
        padded_granular_cor = [
            i + [[-100] * self.T_max for _ in range(max_len - len(i))]
            for i in padded_granular_cor
        ]
        padded_granular_cor = torch.Tensor(
            padded_granular_cor
        ).float()  # padded_granular_cor shape: (B, max_timestep_len, T_max)
        padded_granular_cor = torch.transpose(
            padded_granular_cor, 0, 1
        )  # transposed padded_granular_cor shape: (max_timestep_len, B, T_max)

        embeddings = [b["prompt-embedding"] for b in batch]
        padded_embeddings = [
            i + [torch.zeros(i[0].shape[0])] * (max_len - len(i)) for i in embeddings
        ]
        padded_embeddings = torch.stack(
            [torch.stack(x, dim=0) for x in padded_embeddings], dim=1
        ).float()
