...
def main(configs):
    ...
    _, train_loader = make_dataloader(
        train_set, dataset, collate_fn, configs, split_by_student=True, granular=True
    )
    ...
    if configs.use_transition_model:
        ...
        optimizer_trans = optim.AdamW(
            transition_model.parameters(), lr=configs.trans_lr
        )
        ...
        if configs.use_lstm:
            if configs.use_scheduler_lstm:
                scheduler1 = ReduceLROnPlateau(
                    optimizer_lstm,
                    "min",
                    factor=configs.scheduler_lstm_factor,
                    patience=5,
                )
                schedulers.append(scheduler1)
        if configs.use_scheduler_classifier:
            scheduler2 = ReduceLROnPlateau(
                optimizer_granular, "min", factor=configs.scheduler_granular, patience=5
            )
            schedulers.append(scheduler2)
        if configs.use_transition_model:
            scheduler3 = ReduceLROnPlateau(
                optimizer_trans, "min", factor=configs.scheduler_trans, patience=5
            )
    ...
    for ep in tqdm(range(configs.epochs), desc="epochs"):
        ...
        for idx, batch in enumerate(tqdm(train_loader, desc="training", leave=False)):
            train_log = predict_granular_step(
                idx,
                batch,
                lstm,
                granular_model,
                transition_model=transition_model,
                optimizers_granular=optimizer_granular,
                optimizers_lstm=optimizer_lstm,
                optimizers_trans=optimizer_trans,
                configs=configs,
        ...
        for idx, batch in enumerate(tqdm(valid_loader, desc="validation", leave=False)):
            valid_log = predict_granular_step(
                idx,
                batch,
                lstm,
                granular_model,
        ...
        for idx, batch in enumerate(tqdm(test_loader, desc="testing", leave=False)):
            test_log = predict_granular_step(
                idx,
                batch,
                lstm,
                granular_model,
                ...
                if float(valid_logs[key]) < best_valid_metrics[key]:
                    ...
                    if configs.save_model:
                        ...
                        torch.save(
                            granular_model,
                            os.path.join(
                                configs.model_save_dir, now, "granular_model.pth"
                            ),
                        )
                        if transition_model:
                            torch.save(
                                transition_model.state_dict(),
                                os.path.join(configs.model_save_dir, now, "transition"),
                            )
